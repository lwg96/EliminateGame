cc.Class({
    ...
    dropAndUpdata:function(){//下落动画以及更新位置信息
        ...
        let tempCount = -1;
        for(var i=0;i<this.stars.length;i++){
            cc.log("length = ",this.stars[i].length);
            if (this.stars[i].length == 0){
                tempCount = i;
                for (let col = (i+1); col < this.stars.length; col++){
                    for (let row = 0; row < this.stars[col].length; row++){
                        var act = cc.moveTo(1,this.pSet[col-1][row]);
                        this.stars[col][row].runAction(act);
                        var com=this.stars[col][row].getComponent('Star');
                        com.pos=cc.v2(col-1,row);
                        cc.log("-------------:",com.pos.x);
                    }
                }
            }else{
