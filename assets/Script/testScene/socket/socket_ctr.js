const socketCtr = function(){
    ...
    that.initSocket = function(){
       ...
       _socket.on("notify",function(res){
        ...
        //  "avatar_3","goldcount":1000,"seatindex":2},"callBackIndex":null}
              ...
              var type = res.type
              GlobalEventManager.emit(type,res.data)
          // }
    ...
    */
    ...
    that.onPlayerJoinRoom = function(callback){
        GlobalEventManager.on("player_joinroom_notify",callback)
    }

    that.onPlayerReady = function(callback){
        GlobalEventManager.on("player_ready_notify",callback)
    }

    that.onGameStart = function(callback){
        if(callback){
            GlobalEventManager.on("gameStart_notify",callback)
        }
    }

    that.onChangeHouseManage = function(callback){
        if(callback){
            GlobalEventManager.on("changehousemanage_notify",callback)
        }
    }
    //发送ready消息
    ...
    that.onPushCards = function(callback){
        if(callback){
            GlobalEventManager.on("pushcard_notify",callback)
         }
    }
    ...
    that.onCanRobState = function(callback){
        if(callback){
            GlobalEventManager.on("canrob_notify",callback)
         }
    }
    ...
    that.onRobState = function(callback){
        if(callback){
            GlobalEventManager.on("canrob_state_notify",callback)
         }
    }
    ...
    that.onChangeMaster = function(callback){
        if(callback){
            GlobalEventManager.on("change_master_notify",callback)
         }
    }
    ...
    that.onShowBottomCard = function(callback){
        if(callback){
            GlobalEventManager.on("change_showcard_notify",callback)
         }
    }
    ...
    that.onCanChuCard = function(callback){
        if(callback){
            GlobalEventManager.on("can_chu_card_notify",callback)
        }
    }

    that.onRoomChangeState = function(callback){
        if(callback){
            GlobalEventManager.on("room_state_notify",callback)
        }
    }

    that.onOtherPlayerChuCard = function(callback){
        if(callback){
            GlobalEventManager.on("other_chucard_notify",callback)
        }
    }
    return that
}
